...
UnityHeapExplorer là một công cụ dành cho các nhà phát triển Unity để phân tích sự tiêu thụ bộ nhớ trong Game. Nó giúp xác định và khắc phục các rò rỉ bộ nhớ, phân mảnh và cho phép bạn chụp lại, phân tích và so sánh các Memory Snapshot bằng nhiều phương pháp, chế độ xem, bộ lọc, số liệu và tính năng khác nhau. Bài viết này sẽ giúp bạn hiểu cách sử dụng UnityHeapExplorer để cải thiện quản lý bộ nhớ trong của Game của bạn.
...
- Toolbar: Dùng để chụp, tải, lưu và so sánh các Memory Snapshot, và truy cập cài đặt và trợ giúp.
- Snapshot list: Hiển thị các số liệu thống kê về bộ nhớ, như kích thước, số lượng, tỷ lệ phần trăm, và sự thay đổi của các đối tượng, loại và tham chiếu.
- View list: Hiển thị các chế độ xem để phân tích các Memory Snapshot, như chế độ xem tóm tắt, loại, tham chiếu và bản đồ bộ nhớ.
- View area: Hiển thị nội dung của chế độ xem được chọn, như bảng, biểu đồ và đồ thị.

## Cách chụp một Memory Snapshot

Để chụp một Memory Snapshot trong UnityHeapExplorer, hãy chạy Game của bạn trên Unity Editor hoặc trình chơi độc lập và nhấp vào 'Take Snapshot'. Game sẽ tạm dừng một lúc để chụp Snapshot, sau đó hiển thị trong danh sách Snapshot. Bạn cũng có thể sử dụng phím tắt hoặc một đoạn mã để chụp một Snapshot.

## Cách phân tích một Memory Snapshot

Để phân tích một Memory Snapshot trong UnityHeapExplorer, hãy chọn nó từ danh sách Snapshot và chọn một chế độ xem từ danh sách chế độ xem. Có bốn chế độ xem là:
