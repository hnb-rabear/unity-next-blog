...
Dự án game, không phân biệt quy mô lớn hay nhỏ, đều cần có một Static Database. Tùy thuộc vào quy mô dự án, số lượng Data Table tăng lên, Constants và Id cũng sẽ tăng lên theo. Vấn đề sẽ càng phức tạp nếu không có một giải pháp quản lý hiệu quả.
Vì vậy, mục tiêu ban đầu của công cụ này là để quản lý IDs, Constants và tìm ra cách để có thể thay đổi IDs và Constants ở một nơi duy nhất mà Data Table cũng được cập nhật theo.
...
[Download detail example from GitHub](https://github.com/nbhung100914/ExcelToUnity/blob/main/ExcelToUnity/Document/DataConverterExample.xlsx)
...
![Encryption](/media/attribute.png)
...
# Bài viết này đến đây thôi, trong bài viết kế tiếp chúng ta sẽ học cách sử dụng thông qua một dự án thực tế
